...
export const panCorrectionObject = z.object({
  ...
  addresses: z.object({
    residence: addressSchema.partial().optional(),
    office: addressSchema.partial().optional(),
  }).optional(),

  // UI Support Fields (Added for 5-step standardized UI, not validated or mapped yet)
  ...
  // AO Code (Always required for correction as well usually, but let's make it optional if not specified)
  aoCode: z.object({
